            ...
            <div>
                <div className='flex items-center gap-2'>
                    <Link onClick={(e) => e.stopPropagation()} href={`/profile/${post?.author?.id}`} className="flex items-center gap-3 w-fit hover:underline">
                        ...
                        <span className='flex items-center gap-[2px] ftext-[#333333] dark:text-[#d9d9d9] text-base font-medium break-all'>{post?.author?.username}  <CheckProfile isverified={post?.author?.isverified} /></span>
                    </Link>
