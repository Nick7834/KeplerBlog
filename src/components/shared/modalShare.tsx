...
export const ModalShare: React.FC<Props> = ({
  idPost,
  showModalShare,
  setShowModalShare,
}) => {
  ...
  return (
    <Dialog
      open={showModalShare}
      onOpenChange={(open) => (
        setShowModalShare(open), setCopied(false), setIsCopied(false)
      )}
    >
      <DialogContent className="bg-[#ffffff]/80 dark:bg-[#19191b]/60 backdrop-blur-[12px] rounded-md w-full max-w-[95%] sm:max-w-fit p-4 mx-auto ">
        ...
        <div className="flex items-center justify-center mt-3 w-full overflow-hidden">
          ...
          >
            {shareButtons.map(({ Component, Icon, name }) => (
              <SwiperSlide
                key={name}
                className="!w-auto overflow-hidden [&:not(:last-child)]:mr-5"
              >
                <Component
                  key={name}
                  url={idPost}
                ...
                >
                  <Icon
                    ...
                    round
                  />
                  <span className="text-[#333333] dark:text-[#d9d9d9] text-xs">
                    {name}
                  </span>
                </Component>
              </SwiperSlide>
        ...
        <div className="mt-5 flex items-center gap-2 overflow-hidden">
          ...
          <Button
            variant="secondary"
            onClick={() => handleCopyLink(idPost)}
            className={cn(
              "bg-[#ffffff] dark:bg-[#484848] dark:hover:bg-[#333333]/80 hover:bg-[#d9d9d9]/80",
              copied &&
                "bg-[#7391d5] dark:bg-[#7391d5] hover:bg-[#5772b0] dark:hover:bg-[#5772b0] text-white"
            )}
          >
            {copied ? "Copied" : "Copy"}
          </Button>
