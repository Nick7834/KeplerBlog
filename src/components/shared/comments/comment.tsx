...
export const Comment: React.FC<Props> = ({
  ...
  const indent = Math.min(indentLevel, 5) * (isMobile ? 0.5 : 4);
  ...
  return (
    <div
      className={cn(
        "pt-[15px] pb-[15px] relative z-[1] flex flex-col",
        className
      )}
    ...
    >
        ...
        <div className="flex items-center gap-1 text-[#333333] dark:text-[#d9d9d9] text-base font-medium ml-3">
          <div className="flex items-center gap-[2px]">
            ...
            {comment?.author?.isverified && (
              <CheckProfile isverified={comment?.author?.isverified} />
            )}
          </div>
      ...
      <div className="flex items-center gap-5 mt-2 ml-5">
        {comment?.replies?.length > 0 && (
          <Button
            onClick={() => setIsReplyF(!isReplyF)}
            variant="link"
            className="p-0 h-fit text-[#333333] dark:text-[#d9d9d9] hover:no-underline [&_svg]:size-[20px] flex items-center gap-2"
          >
        ...
        <Button
          onClick={handReply}
          variant="link"
          className="flex gap-2 items-center p-0 h-fit text-[#333333] dark:text-[#d9d9d9] text-base font-medium hover:no-underline"
        >
          <FaRegComment /> Reply
        </Button>
      </div>
      ...
      {comment.author?.id === session?.user?.id && (
        <div className="absolute top-[30px] right-0 flex gap-5">
          <Button
            variant="link"
            className="block p-0 h-fit w-fit"
            onClick={() => setIsUpdate(!isUpdate)}
          >
            <MdCreate className="block translate-y-[-1px]" />
          </Button>
          <Button
            variant="link"
            className="block p-0 h-fit w-fit"
          ...
          >
            <RiDeleteBinLine className="block translate-y-[-1px]" />
          </Button>
        </div>
      )}
    </div>
